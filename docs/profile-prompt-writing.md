...
英文前言只声明本 Run **可用**哪些通道工具。Profile **不要复述**工具定义、不填 `chat_id`、工作区路径。业务仍决定**何时开口**：有结果 / 空转 / 失败时用 `reply_text`、`reply_card`、`no_reply` 还是 `notify_card`，以及卡片标题和按钮。
...
`no_reply` 的 `reason` 应简短说明为什么不打扰管理员。它只用于 Run record 审计，不会发送给用户。入站越界用 `reply_text` 把说明发给用户，不要 `no_reply`。
...
- **把入站卡片写成 `notify_card`**：需要引用回复卡片时用 `reply_card`；`notify_card` 只给有 notify 目标的群公告。
...
| 输入 | 期望 |
| --- | --- |
| 群聊中明确 `@` 机器人并提出职责内请求 | 回复（`reply_text` 或 `reply_card`）；Run succeeded；Delivery delivered |
| 群聊中仅 `@ All` 的无关通知 | 短 `reply_text`：不在职责范围内 |
| 引用机器人上一份结果并补充职责内状态 | 结合引用链处理并回复 |
| 无引用的相似短句 | 按 `persona.md` 的明确性规则处理，不能凭空补上下文 |
| 私聊中的职责内请求 | 回复（`reply_text` 或 `reply_card`） |
