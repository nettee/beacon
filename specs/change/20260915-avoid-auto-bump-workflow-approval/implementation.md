...
- 2026-09-15：通过 GitHub UI 创建 fine-grained PAT `beacon-auto-bump-ci`，仅授权 `nettee/beacon`，Contents read/write、Metadata read-only，并按维护者要求设为无过期时间；保存为 repository secret `AUTO_BUMP_TOKEN`，页面确认更新成功。
