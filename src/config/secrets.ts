    ...
    profiles: z.record(
      ...
      z
        .object({
          feishu: z
            .object({
              app_id: z.string().regex(/^cli_[0-9a-fA-F]{16}$/, "Invalid Feishu app_id"),
...
export async function loadFeishuCredentials(
  ...
  const metadata = await stat(path).catch((error: unknown) => {
    throw new Error(`Cannot inspect Beacon secrets file at ${path}`, { cause: error });
  });
  if (!metadata.isFile()) throw new Error(`Beacon secrets path is not a file: ${path}`);
  ...
  try {
    ...
    throw new Error(`Cannot read Beacon secrets file at ${path}`, { cause: error });
  }
  ...
  try {
    ...
    throw new Error(`Beacon secrets file is not valid JSON: ${path}`, { cause: error });
  }
